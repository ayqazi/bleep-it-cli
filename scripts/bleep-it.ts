...
function help() {
  console.log(`
Usage: npx tsx scripts/bleep-it.ts --input <file> [options]
...
async function main() {
  ...
  if (bleepSound !== 'silence') {
    const bleepPath = path.join(PROJECT_ROOT, 'public', 'bleeps', `${bleepSound}.mp3`);
  ...
  // Load original audio at native quality for bleep processing
